...
var open = process.platform === "linux" ? "xdg-open 2>/dev/null" : "win32" ? "start" : "open";
var openUrl = process.platform === "win32" ? 'https://github.com/settings/tokens/new' : "'https://github.com/settings/tokens/new'";
  ...
  fs.readFile(settingsPath, "utf8", function(error, settings) {
    ...
    if (error) {
      ...
      process.stdin.once("data", function() {
        child.exec(open + " " + openUrl, function(error) {
          if (error) {
            console.log("");
          ...
          readin.question("Enter personal access token: ", function(token) {
            ...
            settings = {token: token, open: error ? null : open, gitConfigs: []};
